...
public sealed class ConvexHullTest
{
    ...
    //added
    public static IList<Point> TestVsNaiveRandomly()
        //
    {
        const int TRIALS = 100000;
        IList<Point> returnList = null;
        for (int i = 0; i < TRIALS; i++)
        {
            int len = rand.Next(20, 400);
            IList<Point> points = new List<Point>();
            //if (rand.NextDouble() < 0.5)
            if (rand.Next(20, 400) < 70)
            {
                for (int j = 0; j < len; j++)
    ...
    public static IList<Point> MakeHullNaive(IList<Point> points)
    {
        if (points.Count <= 1)
    ...
    public static void AssertEquals(IList<Point> a, IList<Point> b)
    {
